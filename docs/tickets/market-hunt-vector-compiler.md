# Market Hunt Vector Compiler Tickets

Draft tickets. Do not publish without explicit approval.

## Feature: Add raw seed lineage compiler

When an agent requests a raw market-hunt vector, I want the compiler to
...
- The compiler loads and validates the 11 versioned YAML seed files in
  `packages/mcp-server/seeds/market-hunt`.
...
- The MCP server always exposes `market_hunt_vector_compiler`.
