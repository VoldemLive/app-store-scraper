...
export const RSS_CHARTS_URL = 'http://ax.itunes.apple.com/WebObjects/MZStoreServices.woa/ws/RSS';

function parseDeveloperId (url) {
...
function cleanApp (app) {
  ...
  if (app['im:artist'].attributes) {
    ...
    if (app['im:artist'].attributes.href.includes('/id')) {
      ...
      developerId = parseDeveloperId(app['im:artist'].attributes.href);
    }
  }
...
function validate (opts) {
  ...
  opts.num = opts.num === undefined ? 50 : opts.num;
  if (!Number.isInteger(opts.num) || opts.num < 1) {
...
function list (opts) {
  return new Promise(function (resolve, reject) {
    ...
    const url = `${RSS_CHARTS_URL}/${opts.collection}/${category}/limit=${opts.num}/json?s=${storeId}`;
