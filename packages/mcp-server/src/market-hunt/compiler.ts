...
export const MARKET_HUNT_CATEGORIES = [
  'domain',
  'problem',
  'persona',
  'situation',
...
export type MarketHuntCategory = typeof MARKET_HUNT_CATEGORIES[number];
export type SeedSpace = Record<MarketHuntCategory, string[]>;
export type SeedLineage = Record<MarketHuntCategory, string>;
...
export type CompilerInfo = {
  compiler_version: string;
  strategy: 'full_random';
  seed_categories: readonly MarketHuntCategory[];
  deterministic_random_seed: true;
};

const FILE_NAMES: Record<MarketHuntCategory, string> = {
  domain: 'domains.yaml',
...
const DEFAULT_SEED_DIRECTORY = fileURLToPath(
  new URL('../../../seeds/market-hunt/', import.meta.url)
);
...
function validateRecords (category: MarketHuntCategory, raw: unknown): string[] {
...
function deterministicIndex (seed: string, category: MarketHuntCategory, length: number): number {
...
function copySeedSpace (seedSpace: SeedSpace): SeedSpace {
  return Object.fromEntries(
    MARKET_HUNT_CATEGORIES.map(category => [category, [...seedSpace[category]]])
  ) as SeedSpace;
}

export class MarketHuntVectorCompiler {
  ...
  async reloadSeedSpace (): Promise<SeedSpace> {
    const entries = await Promise.all(MARKET_HUNT_CATEGORIES.map(async category => {
  ...
  async compileVector (input: CompileVectorInput): Promise<CompileVectorResult> {
    ...
    const lineage = Object.fromEntries(MARKET_HUNT_CATEGORIES.map(category => {
